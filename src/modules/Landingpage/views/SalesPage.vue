...
import CarouselHmo from '../components/carousels/CarouselHmo.vue'
...
import AppIconAccessibility from '@/components/icons/AppIconAccessibility.vue'
import AppIconHeadLightbulb from '@/components/icons/AppIconHeadLightbulb.vue'
import AppIconFileChartCheck from '@/components/icons/AppIconFileChartCheck.vue'
import EnquiryService from '@/services/GenericService/Service'
...
const form = ref<[{ value: (string | number); name: string; }] | null>(null)
...
async function submitHandler() {
  if (validateInputs()) {
    ...
    try {
      let res = await EnquiryService.enquiry(inputValues)
      if (res.status == 200) {
        store.success('Your details were successfully submitted.')
      }
      if (res.status === 200) {
        store.warning('Your details have already been submitted.')
      }
    } catch (error: any) {
      if (error.response?.status === 400) {
        store.error('An error occured while trying to submit your data. Please try again.')
      }
    } finally {
...
.section-title {
  text-align: center;
  @apply text-3xl md:text-5xl font-semibold mb-12;
}
.cards {
  display: flex;
...
.point:is(:nth-child(2), :nth-child(3)) {
  margin-left: 20px;
}
.point-image {
  height: 24px;
...
.plan.light .divider-color {
  border: 1px solid #000000;
}
.plan.light .plan-setup-fee,
.plan.light .plan-price-biling {
  color: #aaaaaa;
}
.plan.light .plan-offer-title,
.plan.light .plan-offer {
  color: #303030;
}
.plan.dark {
  background: #303030;
}
.plan.dark .plan-title,
.plan.dark .plan-price {
  color: #fefefe;
}
.plan.dark .divider-color {
  border: 1px solid #aaaaaa;
}
.plan.dark .plan-setup-fee,
.plan.dark .plan-price-biling {
  color: #888888;
}
.plan.dark .plan-offer-title,
.plan.dark .plan-offer {
  color: #fefefe;
}
td {
  padding: 1rem;
